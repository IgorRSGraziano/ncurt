...
export class UnicodeIncrement {
	...
	public static nextChar = (char: string): string => {
		...
		for (let i = 0; i <= this.scopes.length - 1; i++) {
			const condition = c >= this.scopes[i].start && (i === this.scopes.length - 1 ? c < this.scopes[i].end : c <= this.scopes[i].end);
		...
		if (arrStr[arrStr.length - 1] === this.scopes[this.scopes.length - 1].end) {
			...
			for (let i = arrStr.length - 1; i >= 0; i--) {
				...
				if (arrStr[i] === this.scopes[this.scopes.length - 1].end) {
					...
					if (i === 0) {
						arrStr[0] = this.nextChar(String.fromCharCode(arrStr[0])).charCodeAt(0);
					...
					arrStr[i] = this.nextChar(String.fromCharCode(arrStr[i])).charCodeAt(0);
					nextIncrement = false;
				} else {
					break;
				}
			}
		} else if (arrStr[arrStr.length - 1] < this.scopes[this.scopes.length - 1].end) {
			arrStr[arrStr.length - 1] = this.nextChar(String.fromCharCode(arrStr[arrStr.length - 1])).charCodeAt(0);
		}
