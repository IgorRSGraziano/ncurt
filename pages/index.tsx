...
const Home: React.FC<IHome> = () => {
	...
	const verifyUrlFormat = (): void => {
		const urls: string[] = defaultUrl.current.value.split(";").map((u: string) => u.trim());
		...
		<HomeStyle>
			...
			<CollumAlign>
				...
				<Input
					...
					ref={defaultUrl}
				/>
				<Small>Você pode enviar uma lista de URL's passando um ";" entre elas</Small>
				{status.generated &&
					(status.response?.sucess ? (
						status.response?.urls?.map((el) => (
							<ReturnMessage>
								...
								{"   "}
								<A target="_blank" href={`${window.location.protocol}//${el.url}`}>
									{el.url}
								</A>
								<CopyBtn
									onClick={() => {
				...
				<Input id="urlVerify" placeholder="https://www.site.com.br" ref={destinyUrl} buttonName="Verificar" buttonAction={verifyUrl} />
